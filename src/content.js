let darkMode = false;
const iconSun = `<svg width='40' height='40' viewBox='0 0 40 40' fill='#657786' xmlns='http://www.w3.org/2000/svg'>
...
const iconMoon = `<svg width='40' height='40' viewBox='0 0 40 40' fill='#8899A6' xmlns='http://www.w3.org/2000/svg'>
...
async function getTwittercard() {
    ...
    let imgDlElem = document.getElementById('button-download');
    ...
    if (url.includes('https://twitter.com/') && url.includes('/status/')) {
        ...
        if (await imageExists(`http://twittercard.zixucheah331.ml/image?id=${id}${darkMode ? '&darkMode=true' : ''}`)) {
            ...
            imgDlElem.href = `http://twittercard.zixucheah331.ml/image?id=${id}${darkMode ? '&darkMode=true' : ''}&download=true`;
        } 
